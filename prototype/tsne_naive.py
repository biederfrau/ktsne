...
def compute_pairwise_affinities(dist_sq_one_point, i, sigma=1/np.sqrt(2)):
    enumerators = np.exp(-dist_sq_one_point[:15] / (2*sigma**2))
    denomiators = np.exp(-dist_sq_one_point[:15] / (2*sigma**2))
...
def run_binary_search(dist_sq_one_point, perp, i, tol=1e-5):
    ...
    print("perp_diff = ", perp_diff)
    exit()
    return sigma_hat

class NaiveTSNE():
    def __init__(self, n_epochs=200, perplexity=5, d=2, eta=100, momentum=lambda t: 0.5 if t < 250 else 0.8, save_embeddings=False):
    ...
    def _fit(self, X):
        ...
        for t in range(self.n_iter):
            ...
            for i in range(n):
                ...
                F_rep = np.tile(qijZ[:, i]**2, (self.d, 1)).T * (Y[i, :] - Y) / Z

                dY[i, :] = np.sum(F_attr, 0) - np.sum(F_rep, 0)
...
data = normalize(datasets.load_iris().data)
